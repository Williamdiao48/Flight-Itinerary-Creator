...
void FlightManager::add_flight(const FlightSegment &flight) {
  flights_by_airport[flight.source_airport].insert(flight);
}
...
bool FlightManager::load_flight_data(std::string filename) {
  ...
  while (std::getline(file, line)) {
    ...
    // Check if the data is valid and if so, create a FlightSegment and insert
    // into the set
    if (!ss.fail()) {
      FlightSegment flight(airline, flight_no, source_airport,
                           destination_airport, departure_time, duration_sec,
                           price);
      flights_by_airport[source_airport].insert(
          flight); // Insert into the set for the source airport
    }
  }
  file.close();
  return true;
...
FlightManager::find_flights(std::string source_airport, int start_time,
                            int end_time) const {
  ...
  if (it != flights_by_airport.end()) {
    ...
    auto iter = flights.find_first_not_smaller(
        FlightSegment("", 0, "", "", start_time, 0, 0));
